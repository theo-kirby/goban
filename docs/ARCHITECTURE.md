...
- `Game` — holds the `BoardState3D` and all tool state. A fixed-width shell with a stable toolbar (controls never reflow when options change), a board area, and a status bar.
- `Slice` — renders one z-layer as a 2D SVG goban: grid lines, stones, faint "ghost" dots for stones on adjacent layers, a hover preview stone in the current color, and liberty highlights. Click-to-play.
